...
class NewProjectWithNotesTests(StaticLiveServerTestCase):
    ...
    def test_create_project_with_note(self):
        self.user = get_user_model().objects.create_user(username='alpha', password='alpha', is_superuser=True)
        self.tech = Technician.objects.create(user=self.user, name="Alpha")
        ...
        el_username.send_keys(self.user.username)
        el_password.send_keys(self.user.username)

        el_form.submit()

        el_title = self.selenium.find_element(By.ID, 'id_title')
        el_title.send_keys('Test Project')
        ...
        el_projectnote_maintext.send_keys('First Test Note for Test Project')
