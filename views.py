...
from .forms import (
    ProjectForm,
    ProjectProjectNoteForm,
    ProjectProjectNoteFormset,
    TechnicianForm,
)
...
def send_project_mail(project, request, is_new=False):
    ...
    mail_from = (
        settings.PROSDIB_EMAIL_FROM
        if hasattr(settings, "PROSDIB_FROM_EMAIL")
    ...
    try:
        ...
        messages.add_message(
            request, messages.WARNING, "There was an error sending emails."
        )
...
class ProjectCreate(PermissionRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.POST:
            ...
            context_data["projectnotes"] = ProjectProjectNoteFormset(
    ...
    def get_initial(self):
        tech_emails = [
            tech.user.email
            for tech in Technician.objects.filter(is_current=True).filter(
                user__isnull=False
            )
        ]
        all_recipient_emails = (
            (tech_emails + [self.request.user.email])
            if self.request.user.email not in tech_emails
            else tech_emails
        )
    ...
    def form_valid(self, form):
        ...
        if self.request.POST:
            projectnotes = ProjectProjectNoteFormset(
...
class ProjectUpdate(PermissionRequiredMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.POST:
            ...
            context_data["projectnotes"] = ProjectProjectNoteFormset(
                instance=self.object, initial=[{"submitted_by": self.request.user}]
            )
...
class ProjectList(PermissionRequiredMixin, ListView):
    ...
    def setup(self, request, *args, **kwargs):
        ...
        self.vista_settings["fields"]["latest_update_text"] = {
            "type": "TextField",
            "label": "Latest Major Update Text",
            "available_for": ["columns"],
        }
    ...
    def get_context_data(self, **kwargs):
        ...
        context_data["vistas"] = Vista.objects.filter(
            user=self.request.user, model_name="libtekin.project"
...
class ProjectProjectNoteCreate(PermissionRequiredMixin, CreateView):
    ...
    def get_success_url(self):
        return reverse_lazy(
            "prosdib:project-detail", kwargs={"pk": self.object.project.pk}
        )
...
class TechnicianCreate(PermissionRequiredMixin, CreateView):
    ...
    def get_success_url(self):
        if "opener" in self.request.POST and self.request.POST["opener"] > "":
            return reverse_lazy(
                "prosdib:technician-close", kwargs={"pk": self.object.pk}
            )
        else:
            return reverse_lazy(
                "prosdib:technician-detail", kwargs={"pk": self.object.pk}
            )
...
class TechnicianDetail(PermissionRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        context_data = super().get_context_data(**kwargs)
        context_data["technician_labels"] = {
            field.name: field.verbose_name.title()
            for field in Technician._meta.get_fields()
            if type(field).__name__[-3:] != "Rel"
        }
